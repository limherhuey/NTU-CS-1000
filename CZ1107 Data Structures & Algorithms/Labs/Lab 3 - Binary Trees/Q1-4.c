...
void mirrorTree(BTNode *node)
{
	...
	// base case: when tree is empty, nothing to do
	if (node == NULL)
		return;
	...
	// switch places of left and right subtrees
	temp = node->left;
...
int hasGreatGrandchild(BTNode *node)
{
	...
	// if max height >= 3, this node has great grandchildren
	if (max >= 3)
		printf("%d ", node->item);
...
void printSmallerValues(BTNode *node, int m)
{
	...
	// print smaller values from left and right subtree
	printSmallerValues(node->left, m);
	printSmallerValues(node->right, m);

	// print value in this node if smaller than m
	if (node->item < m)
		printf(" %d", node->item);
}
...
int smallestValue(BTNode *node)
{
	...
	// get smallest values from left and right subtree
	...
	if (l == NULL && r == NULL)
		return node->item;
	// only one subtree: the NULL value cannot be minimum
	else if (l == NULL)
		l = r + 1;
	else if (r == NULL)
		r = l + 1;
