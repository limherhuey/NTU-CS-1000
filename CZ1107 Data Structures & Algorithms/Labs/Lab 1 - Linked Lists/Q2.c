...
int removeNode2(LinkedList *ll,int index)
{
    // if linked list is empty, index is negative, or index is beyond max. index
    if (ll->size == 0 || index < 0 || ll->size - 1 < index)
        return -1;

    ListNode *pre, *node = ll->head;
    ...
    // for indexes > 0, loop to find node at index location
    while (index > 0) {
        pre = node;
        node = node->next;
        index--;
