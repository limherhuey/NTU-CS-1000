...
void in2partialPost(char *, char *); // utility function
...
void in2Pre(char* infix, char* prefix)
{
    ...
    // get partial postfix of the reversed infix
    in2partialPost(infix,prefix);

    // reverse partial postfix = prefix
    strrev(prefix);
}

void in2partialPost(char *infix, char *postfix)
{ /* converts infix to partial postfix */
    Stack s;
    s.head = NULL;
    s.size = 0;
    int i = 0;
    ...
    while (*infix != '\0')
    {
        ...
        else
        {
            ...
            // 1.1. until empty, open bracket on top of stack, or top operator has <= precedence
            // difference from in2Post: pop while top > current, instead of while top >= current
