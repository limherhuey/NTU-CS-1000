...
void appendLL(LinkedList *ll_a , LinkedList *ll_b)
{
	// if both LinkedLists are empty or if ll_b is empty: nothing to do
	if ((ll_a->head == NULL && ll_b->head == NULL) || ll_b->head == NULL)
		return;

	// if ll_a is empty then ll_a = ll_b
	if (ll_a->head == NULL) {
		ll_a->head = ll_b->head;
		ll_a->size = ll_b->size;
	} else {
		// find last node of ll_a
		ListNode *end_a = findNode(ll_a, ll_a->size - 1);

		// attach last node of ll_a to first node of ll_b
		end_a->next = ll_b->head;
		ll_a->size += ll_b->size;
	}
