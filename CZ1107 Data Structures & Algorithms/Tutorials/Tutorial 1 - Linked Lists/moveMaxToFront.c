...
int moveMaxToFront(ListNode **ptrHead)
{
    ListNode *cur = *ptrHead, *max = *ptrHead, *preCur = *ptrHead, *preMax = *ptrHead;

	// traverse linked list once to find largest value
	while (cur != NULL) {
	...
	// if max is already at front
	if (max == *ptrHead)
		return 0;

	// remove max from its position in the linked list
	preMax->next = max->next;

	// move max to the front
	max->next = *ptrHead;
	*ptrHead = max;
