...
void reverseFirstKItems(Queue *q, int k)
{
	...
	if (q->ll.head == NULL || k > q->ll.size || k <= 0)
		return;

	// move first k items to stack
	for (i = 0; i < k; i++) {
		push(&s, dequeue(q));
	}

	// move first k items back to queue - reverses
	while(!isEmptyStack(&s)) {
		enqueue(q, pop(&s));
	}

	// requeue the rest of the size-k items
	for (i = 0; i < j; i++) {
		enqueue(q, dequeue(q));
	}
}

/* MY ORIGINAL FUNCTION - 4 LOOPS
void reverseFirstKItems(Queue *q, int k)
{
	int n = q->ll.size - k, i;
    Stack s;
	...
	// move all queue items into stack
	while (!isEmptyQueue(q)) {
		push(&s, dequeue(q));
	}

	// move all items from stack back into queue (will be reversed)
	while (!isEmptyStack(&s)) {
		enqueue(q, pop(&s));
	}

	// move size-k items into stack
	for (i = 0; i < n; i++) {
		push(&s, dequeue(q));
	}

	// get back original order of size-k items
	for (i = 0; i < n; i++) {
		enqueue(q, pop(&s));
	}
}
