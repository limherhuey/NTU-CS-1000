...
/* Write a C function frontBackSplitLinkedList() that splits the singly linked
list into two sublists — one for the front half, and one for the back half.
If the number of elements is odd, the extra element should go into the front list.
The frontBackSplitLinkedList() prints two lists which contains frontList and backList. */
...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	// nothing to do if ll is empty
	if (ll->size == 0)
		return;

	// find front list's size + declare variables
	int front_size = ll->size / 2.0 + 0.5;
	int i;
	ListNode *temp = ll->head;

	// find last node of front list
	for (i = 0; i < front_size - 1; i++)
		temp = temp->next;
