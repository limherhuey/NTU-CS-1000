...
// _______________ Insertion and Deletion with a Doubly-Linked List for Closed Addressing Hash Table ________________
...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    // table is NULL or key already exists: unsuccessful
    if (Q1Ptr->hSize == 0 || HashSearch(*Q1Ptr, key) != NULL)
        return 0;
    ...
    new->key = key;
    new->next = Q1Ptr->Table[index].head;
    if (new->next != NULL)
        new->next->pre = new;
    new->pre = NULL;
    Q1Ptr->Table[index].head = new;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    // table is NULL or key is not in hash table
    if (Q1Ptr->hSize == 0 || (tmp = HashSearch(*Q1Ptr, key)) == NULL)
        return 0;

    index = Hash(key, Q1Ptr->hSize);

    // reconnect nodes & remove node
    if (tmp->pre == NULL)
        Q1Ptr->Table[index].head = tmp->next;
    else
