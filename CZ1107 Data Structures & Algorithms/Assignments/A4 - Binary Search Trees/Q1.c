...
void inOrderTraversal(BSTNode *root)
{
	...
	// push root and all left descendents onto stack if BST is not empty
	while (cur != NULL) {
		push(&s, cur);
		cur = cur->left;
	}
	...
	while (!isEmpty(&s)) {
		// pop and print a node from the stack
		cur = pop(&s);
		printf("%d ", cur->item);

		// if right child exists, push it and all its left descendents to the stack
		cur = cur->right;
		while (cur != NULL) {
			push(&s, cur);
			cur = cur->left;
