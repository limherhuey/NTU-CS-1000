...
{
    // base case: sum of empty tree = 0
    if (node == NULL)
        return 0;
    
    // sum odd nodes from left & right subtrees, include itself if odd
    return (node->item % 2 == 0 ? 0 : node->item) + sumOfOddNodes(node->left) + sumOfOddNodes(node->right);
}
