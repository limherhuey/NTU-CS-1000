...
int identical(BTNode *tree1, BTNode *tree2)
{
    // base case 1: identical when both are empty
    if (tree1 == NULL && tree2 == NULL)
        return 1;
    // base case 2: not identical when one is empty and the other isn't
    else if ((tree1 != NULL && tree2 == NULL) || (tree1 == NULL && tree2 != NULL))
        return 0;
    
    // both trees are non-empty
    // not identical when current node values are different
    if (tree1->item != tree2->item)
        return 0;

    // check left and right subtrees are identical
